...
const CommentSection = ({videoId}) => {
    ...
    useEffect(() => {
        const fetchVideoComments = async () => {
            try {
                // send a request to youtube API to retrieve comments for the given video ID
                const response = await fetch(`https://www.googleapis.com/youtube/v3/commentThreads?key=${GOOGLE_API_KEY}&textFormat=plainText&part=snippet&videoId=${videoId}&maxResults=50`)
        ...
        <div className="border rounded-lg p-4 bg-white mt-4">
            ...
            <ul className="divide-y divide-gray-200 max-h-[100rem] overflow-y-auto border rounded-lg bg-gray-100 p-4">
                {comments && comments.length > 0 ? (
                    // if comments have been loaded, map over the comments array and render each comment as listitem
                    ...
                    // If no comments have been loaded yet, show shimmer effect to indicate that data is loading
